...
export default function EventsPage() {
  ...
  const getArrangementStatus = (evt: Event) => {
    ...
    // ホテルは hotel_status が「手配済」「不要」 OR ホテル名が入っていれば OK
    const hotelArranged = (s: StaffWithArrangement) => s.hotel_status === "手配済" || s.hotel_status === "不要" || !!s.hotel_name;
                  ...
                  {dialogStaff.length > 0 ? dialogStaff.map((s, i) => {
                    ...
                    return (
                      <div key={`hotel-${s.id}`} className="bg-white rounded border p-2 space-y-1">
                            ...
                            {(["未手配", "手配済", "不要"] as const).map((opt) => {
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => updateStaff("hotel_status", opt)}
                                >
                                  {opt}
                                </button>
                              );
                        ...
                        <Input value={s.hotel_name || ""} onChange={(e) => updateStaff("hotel_name", e.target.value)} placeholder="ホテル名（空欄でも手配済にできます）" className={`h-7 text-xs ${s.hotel_status === "不要" ? "opacity-50" : ""}`} disabled={s.hotel_status === "不要"} />
