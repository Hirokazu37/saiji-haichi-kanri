...
/** 日程表（カレンダービュー）と同じ見た目で催事を選ぶダイアログ */
...
export function EventPickerDialog({ open, onOpenChange, events, selectedId, onSelect }: Props) {
  const today = new Date();
  const [calYear, setCalYear] = useState(today.getFullYear());
  const [calMonth, setCalMonth] = useState(today.getMonth() + 1);

  // 開くたびに、選択中の催事の月（なければ今月）を表示
  ...
  if (open !== prevOpen) {
    ...
    if (open) {
      ...
      const base = sel ? new Date(sel.start_date + "T00:00:00") : new Date();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>日程表から催事を選ぶ</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground ml-auto">催事のバーをクリックすると選択されます</span>
        </div>
                  ...
                  {lanes.map(({ event, laneIdx, startDay, endDay }) => {
                    ...
                    return (
                      <button
                        key={`${event.id}-${wIdx}`}
                        type="button"
                        onClick={() => { onSelect(event.id); onOpenChange(false); }}
