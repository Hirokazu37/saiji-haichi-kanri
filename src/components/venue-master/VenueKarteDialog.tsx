...
type DailyRow = { event_id: string; date: string; amount: number; paypay_amount: number | null; tax_type: "excluded" | "included"; tax_rate: number | null };
type DailyDetail = { date: string; cash: number; paypay: number; total: number }; // 全て税込
...
export function VenueKarteDialog({ open, onOpenChange, venue, segments, canEdit, allMasterKeys, onNotesSaved }: Props) {
  ...
  // 催事ごとの日別売上明細 (expand で表示)。event_id → 日別明細配列
  const [dailyByEvent, setDailyByEvent] = useState<Map<string, DailyDetail[]>>(new Map());
  // 展開中の催事ID Set
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const detailByEvent = new Map<string, DailyDetail[]>();
      const visitByEvent = new Map<string, number>();
      if (ids.length > 0) {
        const [dailyRes, visRes] = await Promise.all([
          supabase.from("event_daily_revenue").select("event_id, date, amount, paypay_amount, tax_type, tax_rate").in("event_id", ids).order("date"),
        ...
        for (const d of (dailyRes.data as DailyRow[]) || []) {
          ...
          const cash = d.amount;
          const paypay = d.paypay_amount ?? 0;
          const cashIncl = toIncluded(cash, d.tax_type, d.tax_rate);
          const paypayIncl = toIncluded(paypay, d.tax_type, d.tax_rate);
          const totalIncl = cashIncl + paypayIncl;
          totalByEvent.set(d.event_id, (totalByEvent.get(d.event_id) || 0) + totalIncl);
          const arr = detailByEvent.get(d.event_id) || [];
          arr.push({ date: d.date, cash: cashIncl, paypay: paypayIncl, total: totalIncl });
          detailByEvent.set(d.event_id, arr);
        }
        for (const v of (visRes.data as { event_id: string; visit_count: number }[]) || []) {
          visitByEvent.set(v.event_id, v.visit_count);
        }
      }
      setDailyByEvent(detailByEvent);
      setExpandedEvents(new Set()); // 開閉状態をリセット
      setRows(events.map((ev) => ({ ev, sales: totalByEvent.get(ev.id) ?? (ev.revenue || 0), visits: visitByEvent.get(ev.id) ?? 0 })));
                  ...
                  <Table className="min-w-[560px]">
                    ...
                    <TableBody>
                      ...
                      {!loading && rows.length === 0 && (
                        <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-6">この店の催事記録がありません。会場名の表記ゆれがある場合は下の「別表記の名寄せ」で追加してください。</TableCell></TableRow>
                      )}
                      {!loading && rows.map(({ ev, sales, visits }) => {
                        const rate = ev.dm_count && visits > 0 ? ((visits / ev.dm_count) * 100).toFixed(1) : null;
                        const detail = dailyByEvent.get(ev.id) || [];
                        const hasDaily = detail.length > 0;
                        const isExpanded = expandedEvents.has(ev.id);
                        // 日別最大額（バーの相対幅表示用）
                        const maxDaily = detail.reduce((m, d) => Math.max(m, d.total), 0);
                        const hasAnyPaypay = detail.some((d) => d.paypay > 0);
                        return (
                          <Fragment key={ev.id}>
                            <TableRow
                              className={hasDaily ? "cursor-pointer hover:bg-muted/50" : ""}
                              onClick={() => hasDaily && toggleExpand(ev.id)}
                            >
                              ...
                              <TableCell className="text-xs whitespace-nowrap">
                                <Link href={`/events/${ev.id}`} onClick={(e) => e.stopPropagation()} className="hover:underline">
                              ...
                              <TableCell className="text-right tabular-nums">
                                ...
                                {hasDaily && <span className="block text-[10px] text-muted-foreground">({detail.length}日)</span>}
                              </TableCell>
                            ...
                            {isExpanded && hasDaily && (
                              <TableRow>
                                <TableCell colSpan={6} className="bg-muted/20 p-2">
                                  <div className="text-[11px] font-semibold text-muted-foreground mb-1">📅 日別売上（税込）{hasAnyPaypay && " ／ 内訳"}</div>
                                  <div className="space-y-1">
                                    {detail.map((d) => {
                                      const widthPct = maxDaily > 0 ? (d.total / maxDaily) * 100 : 0;
                                      ...
                                      return (
                                        <div key={d.date} className="flex items-center gap-2 text-xs">
                                          <span className={`w-24 shrink-0 tabular-nums ${isSun ? "text-red-600" : isSat ? "text-blue-600" : "text-foreground"}`}>
                                            {d.date.slice(5).replace("-", "/")} ({wday})
                                          </span>
                                          <span className="w-24 text-right tabular-nums font-medium">¥{d.total.toLocaleString()}</span>
                                          <div className="flex-1 h-3 bg-white rounded overflow-hidden border" title={`税込¥${d.total.toLocaleString()}`}>
                                            <div className="h-full bg-emerald-400" style={{ width: `${widthPct}%` }}></div>
                                          </div>
                                          {hasAnyPaypay && (
                                            <span className="text-[10px] text-muted-foreground shrink-0 w-32 text-right">
                                              💴{d.cash > 0 ? `¥${d.cash.toLocaleString()}` : "—"} / 📱{d.paypay > 0 ? `¥${d.paypay.toLocaleString()}` : "—"}
                                            </span>
                                          )}
                                        </div>
                                      );
                                    })}
                                    <div className="flex items-center gap-2 text-xs pt-1 border-t mt-1 font-bold">
                                      <span className="w-24 shrink-0">合計</span>
                                      <span className="w-24 text-right tabular-nums text-emerald-800">
                                        ¥{detail.reduce((s, d) => s + d.total, 0).toLocaleString()}
                                      </span>
                                    </div>
                                  </div>
                                </TableCell>
